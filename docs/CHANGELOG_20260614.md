...
Review Tab 内嵌动态使用指南，帮助用户快速上手：

**引导特性：**
- 默认显示「使用指南」卡片 + 快捷指令提示（`/check` · `/verify` · `/style`）
- 切换审查类型时，引导文案动态更新：
  - **幻觉检测**：3 步引导（输入→审查→替换）
  - **数据核对**：4 步引导（输入→参考文件→审查→替换）
  - **风格检查**：4 步引导（输入→目标风格→审查→替换）
- 开始审查出结果后，引导自动隐藏
- 清空结果或再次审查时，引导重新显示
...
| 审查类型 | 引导内容 |
|----------|----------|
| 幻觉检测 | ① 在下方输入或粘贴待审查文本<br>② 点击「开始审查」自动检测无源数据、虚构引用、逻辑矛盾<br>③ 查看问题卡片，点击「替换」修正问题片段 |
| 数据核对 | ① 在下方输入或粘贴待审查文本<br>② 填写「参考文件」路径（支持 Excel/CSV/JSON/MD/DOCX），或点击「浏览」选择<br>③ 点击「开始审查」自动比对文本中的数字与参考数据<br>④ 查看不一致项卡片，点击「替换」修正错误数据 |
| 风格检查 | ① 在下方输入或粘贴待审查文本<br>② 填写「目标风格」（如 b2b_formal / academic / marketing / technical）<br>③ 点击「开始审查」自动检测语气、用词、句式问题<br>④ 查看风格问题卡片，点击「替换」修正问题片段 |
